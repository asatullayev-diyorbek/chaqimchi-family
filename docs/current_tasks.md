...
- [~] **CT-06 boshlandi (Windows 11), 3 ta muammo topildi:**
  ...
  - **Bug #6 (topildi 2026-09-01, tuzatildi):** o'rnatishdan keyin dastur **ilovalar orasida topilmasdi**. Sabab: `.iss`da `[Icons]` bo'limi umuman yo'q edi — Start Menu yorlig'i yaratilmagan, va `ChaqimchiAI Guard Desktop.exe` (tray/status hamrohi)ning SYSTEM xizmatidan farqli o'laroq **hech qanday autostart mexanizmi yo'q edi** — faqat o'rnatish paytida bir marta `[Run]`dan ishga tushardi. Reboot yoki chiqib-kirishdan keyin tray belgisi butunlay yo'qolib, uni qayta topishning imkoni yo'q edi. Tuzatildi: `[Icons]` — Start Menu yorlig'i, ixtiyoriy Desktop yorlig'i (`[Tasks]`), va `{commonstartup}` yorlig'i (har safar kirishda Desktop.exe qayta ishga tushadi). Inno uninstall paytida bularning barchasini avtomatik olib tashlaydi.
  - **Qolgan:** yangi installer bilan **toza** o'rnatish (hotswap emas — Bug #4/#5 real oqimda tasdiqlanadi); Windows 10; haqiqiy reboot orqali autostart (Bug #6 tuzatildi, hali Windows'da tasdiqlanmagan).
